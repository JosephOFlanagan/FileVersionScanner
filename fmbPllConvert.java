...
public class fmbPllConvert
{
    ...
    public fmbxmlPld conversion() throws IOException, InterruptedException
    {
        ...
        for (int i=0; i < pvcsObjects.length; i++)
        {
            ...
            if(("fmb").equals(pvcsObjects[i].substring((pvcsObjects[i].length() - 3))))
            {
                System.out.println("Checking forms and converting them into xml files");
                File currentFile = new File(directory + "/" + pvcsObjects[i]);
                String currentFileString = (directory + "\\" + pvcsObjects[i]);
                //Run Forms2XML for that particular object
                Runtime fmx = Runtime.getRuntime();
                try
                {
                    System.out.println("Converting " + pvcsObjects[i] + " into an xml file");
                    ...
                    File xmlCheck = new File(directory + "/" + pvcsObjects[i].substring(0,(pvcsObjects[i].length() - 4)) + "_fmb.xml");
                    ...
                    for (int k = 0; k < dir_contents.length; k++)
                    {
                        ...
                        {
                            fileFound = true;
                            fmbxmlArray.add(pvcsObjects[i].substring(0,(pvcsObjects[i].length() - 4)) + "_fmb.xml");
                            break;
                        }
                    }
            ...
            else if (("pll").equals(pvcsObjects[i].substring((pvcsObjects[i].length() - 3))))
            {
                ...
                try
                {
                    ...
                    File currentFile = new File(directory + "/" + pvcsObjects[i].substring(0,(pvcsObjects[i].length() - 3)) + "pld");
                    System.out.println("Converting " + pvcsObjects[i] + " into a pld file");
                    Process plxpr = plx.exec("frmcmp userid=drivedev/drivedev@drg1dev module=" + pvcsObjects[i] + " module_type=LIBRARY window_state=minimize script=YES", null, directory);
                    ...
                    for (int k = 0; k < dir_contents.length; k++)
                    {
                        ...
                        if ((currentFile).equals(dir_contents[k]))
                        {
                            ...
                            fileFound = true;
                            pldArray.add(pvcsObjects[i].substring(0,(pvcsObjects[i].length() - 3)) + "pld");
